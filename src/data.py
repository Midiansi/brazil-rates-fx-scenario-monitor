...
HEADERS = {"User-Agent": "Brazil-Rates-FX-Scenario-Monitor/1.0"}


class DataSourceError(RuntimeError):
    """Raised when an official source is unavailable or returns an invalid schema."""


def _request_json(url: str, params: Mapping[str, str], source: str) -> Any:
    # BCB's OData gateway rejects application/x-www-form-urlencoded '+' spaces.
    # RFC 3986 percent-encoding keeps OData filters interoperable across gateways.
...
def _odata_rows(payload: Any, source: str) -> list[Mapping[str, Any]]:
    ...
    if not isinstance(rows, list):
        raise DataSourceError(f"{source} returned a non-list 'value' field.")
    if any(not isinstance(row, Mapping) for row in rows):
        raise DataSourceError(f"{source} returned a malformed observation.")
    return rows
...
def parse_focus_response(payload: Any, expected_indicator: str) -> pd.DataFrame:
    ...
    frame = frame[list(required)].rename(
        columns={
            "Indicador": "Indicator",
            "Data": "Date",
            "DataReferencia": "Reference year",
            "Mediana": "Median",
            "baseCalculo": "Calculation base",
        }
    )
    frame["Date"] = pd.to_datetime(frame["Date"], format="%Y-%m-%d", errors="coerce")
    frame["Reference year"] = pd.to_numeric(frame["Reference year"], errors="coerce")
    frame["Median"] = pd.to_numeric(frame["Median"], errors="coerce")
    frame["Calculation base"] = pd.to_numeric(frame["Calculation base"], errors="coerce")
    frame["Indicator"] = frame["Indicator"].astype("string")
    frame = frame.loc[
        (frame["Indicator"] == expected_indicator) & (frame["Calculation base"] == 0)
    ]
    frame = frame.dropna(subset=["Date", "Reference year", "Median"])
    if frame.empty:
        raise DataSourceError("BCB Focus returned rows but none passed schema and value validation.")
    frame["Reference year"] = frame["Reference year"].astype(int)
    return (
        frame[columns]
        .sort_values(["Reference year", "Date"])
        .drop_duplicates(["Indicator", "Date", "Reference year"], keep="last")
...
def parse_ptax_response(payload: Any) -> pd.DataFrame:
    ...
    frame = frame[list(required)].rename(
        columns={
            "cotacaoCompra": "Buying rate",
            "cotacaoVenda": "Selling rate",
    ...
    frame = frame.dropna(subset=["Timestamp", "Buying rate", "Selling rate"])
    if frame.empty:
        raise DataSourceError("BCB PTAX returned rows but none passed schema and value validation.")
    frame["Date"] = frame["Timestamp"].dt.normalize()
    frame["Midpoint"] = (frame["Buying rate"] + frame["Selling rate"]) / 2.0
    return (
        frame[columns]
...
def parse_sgs_response(payload: Any) -> pd.DataFrame:
    ...
    if any(not isinstance(row, Mapping) for row in payload):
        raise DataSourceError("BCB SGS returned a malformed observation.")
    ...
    frame["Value"] = pd.to_numeric(
        frame["Value"].astype("string").str.replace(",", ".", regex=False), errors="coerce"
    )
    frame = frame.dropna(subset=columns)
    if frame.empty:
        raise DataSourceError("BCB SGS returned rows but none passed schema and value validation.")
...
def parse_fred_csv(text: str, series_id: str) -> pd.DataFrame:
    ...
    _require_columns(frame, {"observation_date", series_id}, f"FRED {series_id}")
    if frame.empty:
        return pd.DataFrame(columns=columns)
    frame = frame[["observation_date", series_id]].rename(
...
def fetch_ptax(start_date: date, end_date: date) -> pd.DataFrame:
    url = (
        f"{BCB_PTAX_URL}CotacaoDolarPeriodo"
...
def fetch_fred_series(series_id: str, start_date: date) -> pd.DataFrame:
    allowed = {"DFEDTARL", "DFEDTARU", "DGS2", "DGS10"}
    if series_id not in allowed:
        raise ValueError(f"Unsupported FRED series: {series_id}.")
    try:
        response = requests.get(
            FRED_GRAPH_URL,
            params={"id": series_id, "cosd": start_date.strftime("%Y-%m-%d")},
            headers=HEADERS,
            timeout=REQUEST_TIMEOUT,
        )
        response.raise_for_status()
    except requests.RequestException as exc:
        raise DataSourceError(f"FRED {series_id} request failed: {exc}") from exc
